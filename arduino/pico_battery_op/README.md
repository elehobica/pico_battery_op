...
| File in `src/` | Origin (pico-extras, tag `sdk-2.3.0`) |
|---|---|
| `pbo_vendor/pbo_sleep.c`      | `src/rp2_common/pico_sleep/sleep.c` |
| `pbo_vendor/pbo_sleep.h`      | `src/rp2_common/pico_sleep/include/pico/sleep.h` |
| `pbo_vendor/pbo_rosc_extra.c` | `src/rp2_common/hardware_rosc_extra/rosc_extra.c` |
| `pbo_vendor/pbo_rosc_extra.h` | `src/rp2_common/hardware_rosc_extra/include/hardware/rosc_extra.h` |
...
function is prefixed with `pbov_`** (for example `sleep_power_up` -> `pbov_sleep_power_up`). Their
include guards are renamed as well. `pico_battery_op.cpp` maps the prefixed names back to the SDK
names locally, so this library can safely coexist with other RP2040 sleep / dormant libraries.

Everything else these files need is already provided by the core: `hardware_clocks`, `hardware_irq`,
`pico_aon_timer`, `hardware_xosc`, `hardware_pll`, `hardware_sync`, `pico_runtime_init`, plus
`hardware_rosc` (pulled in by `pico_low_power`) and `hardware_powman` (pulled in by
`pico_aon_timer`, RP2350 only).
