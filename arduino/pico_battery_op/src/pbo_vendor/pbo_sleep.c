...
bool pbov_dormant_source_valid(dormant_source_t dormant_source)
{
...
void pbov_sleep_run_from_dormant_source(dormant_source_t dormant_source) {
    assert(pbov_dormant_source_valid(dormant_source));
...
void pbov_sleep_goto_sleep_until(struct timespec *ts, aon_timer_alarm_handler_t callback)
{
    ...
    // We should have already called the pbov_sleep_run_from_dormant_source function
    // This is only needed for dormancy although it saves power running from xosc while sleeping
    //assert(pbov_dormant_source_valid(_dormant_source));
