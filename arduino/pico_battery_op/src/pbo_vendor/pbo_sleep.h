 ...
 */
void pbov_sleep_run_from_dormant_source(dormant_source_t dormant_source);

/*! \brief Set the dormant clock source to be the crystal oscillator
 *  \ingroup hardware_sleep
 */
static inline void pbov_sleep_run_from_xosc(void) {
    pbov_sleep_run_from_dormant_source(DORMANT_SOURCE_XOSC);
}

#if !PICO_RP2040
static inline void pbov_sleep_run_from_lposc(void) {
    pbov_sleep_run_from_dormant_source(DORMANT_SOURCE_LPOSC);
}
#endif
 ...
 *  \ingroup hardware_sleep
 */
static inline void pbov_sleep_run_from_rosc(void) {
    pbov_sleep_run_from_dormant_source(DORMANT_SOURCE_ROSC);
}
 ...
 */
void pbov_sleep_goto_sleep_until(struct timespec *ts, aon_timer_alarm_handler_t callback);

/*! \brief Send system to sleep for a specified duration in milliseconds. This provides an alternative to pbov_sleep_goto_sleep_until
 ...
 */
bool pbov_sleep_goto_sleep_for(uint32_t delay_ms, hardware_alarm_callback_t callback);
 ...
 * \param callback Function to call on wakeup.
 */
void pbov_sleep_goto_dormant_until(struct timespec *ts, aon_timer_alarm_handler_t callback);
...
void pbov_sleep_goto_dormant_until_pin(uint gpio_pin, bool edge, bool high);
 ...
 * \param gpio_pin The pin to provide the wake up
 */
static inline void pbov_sleep_goto_dormant_until_edge_high(uint gpio_pin) {
    pbov_sleep_goto_dormant_until_pin(gpio_pin, true, true);
}
 ...
 * \param gpio_pin The pin to provide the wake up
 */
static inline void pbov_sleep_goto_dormant_until_level_high(uint gpio_pin) {
    pbov_sleep_goto_dormant_until_pin(gpio_pin, false, true);
}
