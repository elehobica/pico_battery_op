...
// LED blink rate: 1 Hz on a fresh startup, 2 Hz after resuming from a DeepSleep.
static bool resumed_from_sleep = false;

// Turn the LED off just before the library enters DeepSleep (a Sleep keeps
// PboStateActive). This keeps the LED dark while sleeping, regardless of the
...
// Just after waking from DeepSleep; switch the blink rate to 2 Hz.
...
int main(void)
{
    ...
    // output). Callbacks handle the LED around DeepSleep.
    ...
    while (true) {
        ...
        // fresh startup, 2 Hz (250 ms on / 250 ms off) after resuming from a DeepSleep.
